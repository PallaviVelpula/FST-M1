...
select COUNT(distinct salesman_id) salesmen from orders;

select customer_id, MAX(purchase_amount) from orders GROUP BY customer_id;

select salesman_id, MAX(purchase_amount) max_amount from orders where order_date = TO_DATE('2012/08/17', 'YYYY/MM/DD')  
    GROUP BY salesman_id, order_date;

select customer_id, MAX(purchase_amount) max_amount from orders group by customer_id HAVING MAX(purchase_amount)  
